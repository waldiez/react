...
const nodesWithoutData = nodes.map((node: Node) => {
    ...
    if (nodeType === "agent") {
        ...
        if (agentType === "user") {
            agents.users.push(
                WaldiezSourceUserProxy.fromJSON(jsonData, agentType, node.data.label as string),
            );
        } else if (agentType === "assistant") {
            agents.assistants.push(
                WaldiezSourceAssistant.fromJSON(jsonData, agentType, node.data.label as string),
            );
