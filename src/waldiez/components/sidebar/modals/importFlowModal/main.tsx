...
export const ImportFlowModal = (props: ImportFlowModalProps) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Import Flow" dataTestId={`import-flow-modal-${flowId}`}>
        ...
        >
          ...
          <WizardStep id={`import-flow-modal-preview-step-${flowId}`} title="Preview Flow">
            {state.loadedFlowData ? (
              <FlowDataPreviewStep flowId={flowId} state={state} onStateChange={onStateChange} />
            ) : null}
          </WizardStep>
        </Wizard>
      </div>
