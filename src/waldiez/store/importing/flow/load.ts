...
export const loadFlow: (
  ...
  if (items.everything) {
    ...
    mergedFlow = items.override
      ...
      : {
          ...mergedFlow,
          name: FlowStore.mergeFlowName(currentFlow.name, newFlow.name, currentFlow.nodes, currentFlow.edges),
    ...
    if (items.requirements) {
      mergedFlow.requirements = FlowStore.mergeRequirements(currentFlow.requirements, newFlow.requirements);
    }
